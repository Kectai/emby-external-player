...
- `CustomPlayers`：现有自定义入口后会保留一个“添加播放器”配置组，填写并保存后自动生成下一个；删除时关闭条目并清空应用名称和 URL 模板后保存。启用且平台匹配的入口与内置播放器显示在同一列表中，并带“自定义播放器”标识。应用名称按输入原样显示，模板语法见客户端说明。
...
- 详情页入口位置、样式或文字异常：升级到 1.3.1。入口不再使用 `detailButton-autotext`，会直接复用 Emby 的圆角矩形按钮结构，并固定插在“播放/从头开始”右侧。
- 配置页保存按钮显示 `save`：升级到 1.3.1，并强制刷新 Emby Web。插件只在自己的配置页按客户端语言修正为“保存 / 儲存 / Save”。
- 配置页看不到自定义播放器：1.3.1 已移除当前 Emby Web 无法稳定渲染的 `DxDataGrid`，改用官方动态子集合；升级并重启后会显示现有条目及一个“添加播放器”配置组。
