...
- `CustomPlayers`：在表格中选择“添加播放器”，可按需新增、编辑或删除自定义 URL Scheme 入口；启用且平台匹配的入口与内置播放器显示在同一列表中，并带“自定义播放器”标识。应用名称按输入原样显示，模板语法见客户端说明。
...
- 详情页入口样式或文字异常：升级到 1.3.0。入口不再使用 `detailButton-autotext`，会直接复用 Emby 的圆角矩形按钮结构，并跟随相邻播放/继续播放按钮的主题色与响应式状态。
- 配置页保存按钮显示 `save`：升级到 1.3.0，并强制刷新 Emby Web。插件只在自己的配置页按客户端语言修正为“保存 / 儲存 / Save”。
