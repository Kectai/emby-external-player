...
- 如果媒体版本和字幕两行被拉成大文本框、上下间距过大，或左右边缘没有与播放器列表对齐，请升级到 1.4.7。新版显式覆盖 Emby 延迟加载的下拉框高度和容器外边距，并统一内容内缩。
- 如果关闭状态已经对齐，但 macOS 展开的系统下拉菜单仍向左右外扩，请升级到 1.4.8。该外扩属于系统原生 `<select>` 弹层，网页 CSS 无法约束；新版使用与 Emby 主题融合的自定义可访问列表，展开后与字段严格等宽。升级 DLL 并重启服务器后，再对 Emby Web 执行一次强制刷新。
- 如果“打开”下方重复出现作用相同的重试链接，或支持字幕的播放器没有识别所选 SRT/ASS，请升级到 1.4.9。新版删除重复链接，为字幕中转地址保留真实扩展名，并按播放器能力启用字幕选择；IINA 官方应用链接不支持外部字幕参数，因此会显示不可用说明。
