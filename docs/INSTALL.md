...
- `CustomPlayers`：最多配置三个自定义 URL Scheme 入口；启用且平台匹配的入口与内置播放器显示在同一列表中，并带“自定义播放器”标识。应用名称按输入原样显示，模板语法见客户端说明。
...
- 详情页只显示外部播放图标：升级到 1.2.2。Emby 桌面样式会默认隐藏 `detailButton-autotext` 的文字，新版对插件按钮单独恢复本地化标签。
