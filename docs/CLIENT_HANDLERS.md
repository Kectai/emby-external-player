...
配置页通过 Emby 官方动态子集合管理自定义入口，不再预置三个固定槽位。现有入口后始终保留一个“添加播放器”配置组；填写并保存后会自动生成下一个空白组。删除入口时关闭它并清空应用名称和 URL Scheme 模板后保存。每个入口可设置启用状态、平台、官方应用名称和 URL Scheme 模板。应用名称完全按输入显示，不做首字母大写或其他品牌名转换。
