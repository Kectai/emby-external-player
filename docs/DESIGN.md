...
弹窗使用插件自己的命名空间和可访问性逻辑，同时复用 Emby 已加载的 `dialog`、`formDialog`、`emby-button`、`emby-select` 等公共样式类及主题 CSS 变量；不调用未公开的对话框 JavaScript 模块。

弹窗内容：

- 本地化标题与当前媒体标题。
- 按平台过滤后的播放器单选列表，默认播放器用独立于焦点的主题色边框和勾选标识。
- 内置播放器与服务端配置的自定义播放器标识；应用名称保持原样。
- 媒体版本、外挂字幕与“从上次位置继续”选项。
- 统一的“打开”和“取消”底部动作。
- 错误提示和再次尝试链接。
...
1. 用户选择播放器；选中态通过 `aria-checked` 与 CSS 类持久表示，不能借用 `:focus` 或 `:focus-visible`。
2. 用户点击统一“打开”按钮时调用 `/Resolve`。
3. 得到 `launchUrl` 后设置 `window.location.href`。
4. 如果浏览器因异步请求丢失用户激活状态而阻止自定义协议，显示一个真实的 `<a href="custom-scheme:...">打开播放器</a>` 供用户再次点击。
5. 不自动循环尝试多个协议，避免浏览器反复弹窗。
