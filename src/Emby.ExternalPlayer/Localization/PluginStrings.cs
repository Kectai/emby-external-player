...
public static class PluginStrings
{
    private static readonly IReadOnlyDictionary<string, string> English =
        new Dictionary<string, string>(StringComparer.Ordinal)
        {
            ...
            [nameof(CustomPlayersDescription)] = "Optional URL handlers. Application names are displayed exactly as entered. Templates support {url}, {title}, {subtitle}, and {start}.",
    ...
    private static readonly Dictionary<string, string> SimplifiedChinese =
        new Dictionary<string, string>(StringComparer.Ordinal)
        {
            ...
            [nameof(CustomPlayersDescription)] = "可选的 URL 处理器。应用名称会完全按输入显示；模板支持 {url}、{title}、{subtitle} 和 {start}。",
