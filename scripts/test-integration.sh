...
: "${EMBY_INTEGRATION_DASHBOARD_INDEX:?Set EMBY_INTEGRATION_DASHBOARD_INDEX to the isolated dashboard-ui/index.html fixture.}"
...
case "${EMBY_INTEGRATION_DASHBOARD_INDEX}" in
    "${project_root}"/.local/*) ;;
    *)
        echo "Refusing to test: dashboard index.html must be inside ${project_root}/.local." >&2
        exit 2
        ;;
esac
