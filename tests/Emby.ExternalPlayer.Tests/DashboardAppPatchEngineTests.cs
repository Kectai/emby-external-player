...
[TestClass]
public sealed class DashboardAppPatchEngineTests
{
    [TestMethod]
    public void Apply_LoadsModuleThroughEmbysPluginPhase()
    {
    ...
    public void Apply_IsCompatibleWithVerifiedOfficialEmbyAppJs(string version)
    {
        ...
        {
            Assert.Inconclusive(
                "The optional full Emby app.js fixture is not present in the project-local test directory.");
        }
    ...
    [TestMethod]
    public void Apply_IsIdempotentAndReversible()
    {
        const string source = "before;Promise.all(list.map(loadPlugin));after";
        var once = DashboardPatchEngine.Apply(source);
        var twice = DashboardPatchEngine.Apply(once.Content);
        var removed = DashboardPatchEngine.Remove(once.Content);
